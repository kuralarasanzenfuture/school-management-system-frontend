...
//     // School
//     formData.append("school_id", isAdmin ? data.school_id : schoolId);
...
//     formData.append(
//       "current_address_same_as_permanent",
//       data.current_address_same_as_permanent ? "1" : "0",
//     );
...
//     formData.append("ifsc_code", data.ifsc_code.toUpperCase());
...
// Guards against double-prefixing in case a stored value is ever already
// an absolute URL (e.g. https://... or a data: URI) rather than a
// server-relative path.
function resolveExistingUrl(apiUrl, value) {
  if (!value) return "";
  if (
    /^https?:\/\//i.test(value) ||
 ...
 * axios interceptor forces one).
 *
 * `designation` and `department` are submitted as their plain text
 * `name` (matching how EmployeeTable already displays them directly,
 * with no id-to-name lookup) — not as foreign key ids.
 *
 * @param {object|null} initialData
 * @param {(formData: FormData) => void} onSubmit
 * @param {() => void} onCancel
...
export default function EmployeeForm({
  ...
  // Local object: URLs for newly-picked files (or resolved existing
  // record URLs), so we can show an instant thumbnail/lightbox.
  const [previewUrls, setPreviewUrls] = useState({});
  const [lightboxUrl, setLightboxUrl] = useState(null);
  ...
  // Resets the form ONLY when switching between add/edit targets (a
  // different record, or add -> edit / edit -> add) — NOT whenever
  // schoolId/isAdmin happen to change value. schoolId comes from an async
  // auth fetch and can flip from undefined -> a real value well after
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setPreviewUrls({
        photo_url: resolveExistingUrl(API_URL, initialData.photo_url),
        passport_size_photo_url: resolveExistingUrl(
          API_URL,
          initialData.passport_size_photo_url,
        ),
        aadhaar_card_url: resolveExistingUrl(
          API_URL,
          initialData.aadhaar_card_url,
        ),
        pan_card_url: resolveExistingUrl(API_URL, initialData.pan_card_url),
        signature_url: resolveExistingUrl(API_URL, initialData.signature_url),
        degree_certificate_url: resolveExistingUrl(
          API_URL,
          initialData.degree_certificate_url,
        ),
        experience_certificate_url: resolveExistingUrl(
          API_URL,
          initialData.experience_certificate_url,
        ),
      });
    } else {
  ...
  // Separately: once `schoolId` becomes available for a non-admin who has
  ...
  const set = (key) => (e) => {
    ...
    if (key === "salary") {
      // Allow digits and at most one decimal point (matches step="0.01"
      // on the input) instead of stripping the decimal point entirely.
    ...
    <>
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        {isAdmin && (
          <div className="flex flex-col gap-1.5">
            <label className="ep-field-label text-[13px] font-medium">
              School <span className="ep-field-required">*</span>
            </label>
            <select
              className={`ep-input w-full rounded-lg px-3.5 py-2.5 text-[14px] outline-none transition-all duration-200 ${errors.school_id ? "ep-input-error" : ""}`}
              value={data.school_id}
              onChange={set("school_id")}
              disabled={schoolsLoading}
            >
            ...
            <div className="h-4">
              {errors.school_id && (
                <p className="ep-field-error text-[11px]">{errors.school_id}</p>
              )}
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3">
            {DOCUMENT_FIELDS.map(({ key, label }) => {
              ...
              // Prefer the freshly-picked file's local preview; fall back
              // to the resolved existing URL if it looks like an image.
              const previewUrl = previewUrls[key] || null;

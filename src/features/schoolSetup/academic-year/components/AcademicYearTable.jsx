...
/* ────────────────────────────────────────────────────────────────
   AcademicYearTable
   Props
     academicYears  array    full (unsliced) list — pagination is
                             handled internally by usePagination
     onEdit         fn(year)
     onDelete       fn(id)
...
export default function AcademicYearTable({
  ...
  /* usePagination slices `academicYears` and gives back
     only the rows for the current page.                  */
  const { pagedData, currentPage, pageSize, totalItems, setPage, setPageSize } =
    usePagination({ data: academicYears, initialSize: initialPageSize });
  ...
  return (
    <div className="ay-table-card rounded-2xl overflow-hidden">
      ...
      {/* ── Pagination ── rendered only when there is data ── */}
      <Pagination

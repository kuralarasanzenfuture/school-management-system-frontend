...
import ClassSectionTable from "../components/ClassSectionTable.jsx";
import ClassSectionModal from "../components/ClassSectionModal.jsx";
import DeleteClassSectionModal from "../components/DeleteClassSectionModal.jsx";
import StatusFilterDropdown from "../components/StatusFilterDropdown";
import {
  fetchClassSections,
  addClassSection,
...
const ClassSectionPage = () => {
  ...
  // ClassSectionTable calls onDelete(id) — matching every other table in
  // this app. DeleteClassSectionModal needs the full record (class_name,
  // section_name) to show a meaningful confirmation, so look it up here
  // rather than changing the table's contract. Same fix applied to
  // School / AcademicYear / Class / Section pages.
  const openDeleteModal = (id) => {
    const item = classSections.find((cs) => String(cs.id) === String(id));
    setDeleteTarget(item ?? null);
  };

  const closeDeleteModal = () => {
    setDeleteTarget(null);
    dispatch(fetchClassSections()); // reload list after a successful delete
  };

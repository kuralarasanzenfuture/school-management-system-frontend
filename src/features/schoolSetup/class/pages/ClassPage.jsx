...
const ClassPage = () => {
  ...
  // ClassTable calls onDelete(id) — matching every other table in this
  // app. DeleteClassModal needs the full record (name, school_name) to
  // show a meaningful confirmation, so look it up here rather than
  // changing the table's contract. Same fix applied to
  // SchoolProfilePage / AcademicYearPage after the empty-name bug there.
  const openDeleteModal = (id) => {
    const cls = classes.find((c) => String(c.id) === String(id));
    setDeleteTarget(cls ?? null);
  };

  const closeDeleteModal = () => {
    setDeleteTarget(null);
    dispatch(fetchClasses()); // reload list after a successful delete
  };

...
// import BulkAssignSubjectsModal from "../components/BulkAssignSubjectsModal.jsx";
// import {
//   fetchClassSubjects,
...
//   // fetchClassSubjects takes NO arguments — always returns everything, so
//   // school scoping happens client-side (below), not via the fetch call.
//   useEffect(() => {
...
//         ? `${cx.class_name} ${cx.section_name || ""} ${cx.subject_name} ${cx.teacher_name || ""}`
//             .toLowerCase()
//             .includes(term)
//         : true;
...
//             Map subjects (and teachers) to a class section for its academic
//             year.
//           </p>
//         </div>
...
import ClassSubjectTable from "../components/ClassSubjectTable.jsx";
import ClassSubjectModal from "../components/ClassSubjectModal.jsx";
import DeleteClassSubjectModal from "../components/DeleteClassSubjectModal.jsx";
import BulkAssignSubjectsModal from "../components/BulkAssignSubjectsModal.jsx";
import {
  fetchClassSubjects,
  createClassSubjectThunk,
...
const ClassSubjectPage = () => {
  ...
  // ClassSubjectTable calls onDelete(id) — matching every other table in
  // this app. DeleteClassSubjectModal needs the full record (class_name,
  // section_name, subject_name, teacher_name) to show a meaningful
  // confirmation, so look it up here rather than changing the table's
  // contract. Same fix applied to every other module's page.
  const openDeleteModal = (id) => {
    const item = classSubjects.find((cx) => String(cx.id) === String(id));
    setDeleteTarget(item ?? null);
  };

  const closeDeleteModal = () => {
    setDeleteTarget(null);
    dispatch(fetchClassSubjects()); // reload list after a successful delete
  };

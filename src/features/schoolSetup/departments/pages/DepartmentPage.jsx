...
export default function DepartmentPage() {
  ...
  const toggleSelectAll = (event) => {
    setSelectedIds(event.target.checked ? pagedRows.map((dept) => dept.id) : []);
  };
  ...
  const activeCount = departments.filter((dept) => dept.status !== "inactive").length;
    ...
    <div className="dp-page min-h-screen p-5 sm:p-6">
      ...
      {!loading && !error && viewMode === "table" && (
        <div className="dp-table-card rounded-2xl overflow-hidden">
          {/* Card header */}
          <div className="dp-table-header flex items-center justify-between px-5 py-4">
            <h2 className="dp-table-title text-[15px] font-bold">
              Department directory
            </h2>
            <button className="dp-export-btn inline-flex items-center gap-2 px-3.5 py-2 rounded-lg text-[12.5px] font-semibold transition-colors">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
                ...
                <tr className="dp-thead text-[11.5px] uppercase tracking-wide">
                  <th className="px-5 py-3 w-10">
                    <input
                      ...
                      checked={pagedRows.length > 0 && pagedRows.every((dept) => selectedIds.includes(dept.id))}
                    />
                  </th>
                  <th className="px-3 py-3 font-semibold">Name <SortIcon /></th>
                  <th className="px-3 py-3 font-semibold">Description</th>
                  <th className="px-3 py-3 font-semibold">School ID <SortIcon /></th>
              ...
              <tbody>
                {pagedRows.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="dp-empty-state px-5 py-12 text-center text-[13.5px]">
                    ...
                    <tr key={department.id} className="dp-row transition-colors">
                      ...
                      <td className="px-3 py-3.5 max-w-[280px]">
                        <p className="dp-cell-secondary text-[13px] truncate">
                          {department.description || <span className="dp-cell-muted">—</span>}
                        </p>
                      </td>
                      <td className="px-3 py-3.5">
                        <span className="dp-badge">{department.school_id}</span>
                      </td>
                      <td className="px-3 py-3.5">
              ...
              {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-[13px] font-semibold transition-colors ${
                    pageNumber === currentPage ? "dp-page-btn-active" : "dp-page-btn"
                  }`}
                >
                  {pageNumber}
                </button>
      ...
      {!loading && !error && viewMode === "grid" && (
        <>
          {pagedRows.length === 0 ? (
            <div className="dp-table-card rounded-2xl p-12 text-center">
              <p className="dp-empty-state text-[13.5px]">No departments found.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {pagedRows.map((department) => (
                <div key={department.id} className="dp-stat-card rounded-2xl p-5 flex flex-col gap-3">
                  ...
                  <div>
                    <p className="dp-cell-primary text-[14px] font-bold">{department.name}</p>
                    <p className="dp-cell-muted text-[12.5px] mt-1 line-clamp-2 leading-relaxed">
                      {department.description || "No description provided."}
                    </p>
                  </div>
                  <div className="mt-auto pt-2" style={{ borderTop: "1px solid var(--divider)" }}>
                    <span className="dp-cell-muted text-[11.5px]">School ID: </span>
                    <span className="dp-cell-secondary text-[11.5px] font-semibold">{department.school_id}</span>
                  </div>
                </div>
              ))}
            </div>

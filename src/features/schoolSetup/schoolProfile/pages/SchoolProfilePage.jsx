...
const SchoolProfilePage = () => {
  ...
  const closeDeleteModal = () => {
    ...
    dispatch(fetchSchools()); // reload list after a successful delete
  };

  // SchoolTable calls onDelete(id) — matching every other table in this
  // app (RoleTable, UserTable, ClassTable, etc all pass just the id).
  // DeleteSchoolModal needs the full school object to show its name, so
  // look it up here rather than changing SchoolTable's contract.
  const openDeleteModal = (id) => {
    const school = (schools || []).find(
      (s) => String(s.id) === String(id),
    );
    setDeleteTarget(school ?? null);
  };

...
const SectionPage = () => {
  ...
  // SectionTable calls onDelete(id) — matching every other table in this
  // app. DeleteSectionModal needs the full record (name, class_id/name)
  // to show a meaningful confirmation, so look it up here rather than
  // changing the table's contract. Same fix applied to School /
  // AcademicYear / Class pages after the empty-name bug there.
  const openDeleteModal = (id) => {
    const section = sections.find((s) => String(s.id) === String(id));
    setDeleteTarget(section ?? null);
  };

  const closeDeleteModal = () => {
    setDeleteTarget(null);
    dispatch(fetchSections({ schoolId: schoolId, classId: classFilter })); // reload list after a successful delete
  };

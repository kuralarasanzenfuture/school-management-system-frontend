...
//  *
//  * Student attendance tab — uses Redux state management via the
//  * `fetchAttendanceByStudent` async thunk.
//  *
//  * API: GET /api/students-attendance/student/:studentId
...
//  *
//  * State flow:
//  *   dispatch(fetchAttendanceByStudent({ studentId, filters }))
//  *   → state.studentAttendance.attendanceByStudent  (full API response)
//  *   → state.studentAttendance.loading
//  *   → state.studentAttendance.error
//  *
//  * Sub-components:
...
// import {
//   Calendar, List, LayoutGrid, RefreshCw,
//   UserCheck, UserX, Clock, Umbrella, CalendarOff, CalendarDays,
// } from "lucide-react";
...
//   /* ── Redux state ── */
//   const { attendanceByStudent: data, loading, error } = useSelector(
//     (s) => s.studentAttendance,
//   );
...
//   /* ── Build filters object for the thunk ── */
...
//   const dailyRows = useMemo(() => data?.data?.daily ?? [], [data]);
//   const periodRows = useMemo(() => data?.data?.period ?? [], [data]);
...
/**
 * StudentAttendanceTab.jsx  — uses existing Redux thunk
 *
 * Dispatch: fetchAttendanceByStudent({ studentId, filters })
 * State:    state.studentAttendance.attendanceByStudent
 *           state.studentAttendance.loading
 *           state.studentAttendance.error
 *
 * Sub-components:
...
export default function StudentAttendanceTab({ studentId }) {
  ...
  /* ── Redux state ── */
  const {
    attendanceByStudent: apiResponse,  // full API response object
    loading,
    error,
  } = useSelector((s) => s.studentAttendance);
  ...
  const load = useCallback(() => {
    ...
    dispatch(fetchAttendanceByStudent({
      ...
      filters: {
        ...
        to_date:         ymLast(year, month),
        attendance_type: attendType,
        // academic_year_id: omitted → backend defaults to current year
      },
    }));
  }, [dispatch, studentId, year, month, attendType]);

  useEffect(() => { load(); }, [load]);

  /* ── Normalise API response ── */
  // The thunk stores whatever the service returns as-is.
  // Response shape: { admission_id, summary, data: { daily[], period[] } }
  // Guard against empty / 203-no-body responses.
  const dailyRows  = useMemo(() => apiResponse?.data?.daily  ?? [], [apiResponse]);
  const periodRows = useMemo(() => apiResponse?.data?.period ?? [], [apiResponse]);
  const summary    = useMemo(() => apiResponse?.summary      ?? {}, [apiResponse]);
  ...
  /* ── Type filter → auto-switch view ── */
  const handleTypeChange = (type) => {
    setAttendType(type);
    if (type === "daily")        setViewMode("daily");
    else if (type === "period")  setViewMode("period");
    else                         setViewMode("calendar");
  };

  const hasData = dailyRows.length > 0 || periodRows.length > 0;
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="sd-att-table-card rounded-2xl px-4 py-3 flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-1">
          <button onClick={goPrev}
            className="sd-doc-view-btn w-8 h-8 rounded-lg flex items-center justify-center text-[16px] font-bold">‹</button>
          <span className="sd-att-cell text-[13.5px] font-semibold px-2 min-w-[110px] text-center">
            {MONTHS_SHORT[month]} {year}
          </span>
          <button onClick={goNext} disabled={isCurrentMonth}
            className="sd-doc-view-btn w-8 h-8 rounded-lg flex items-center justify-center text-[16px] font-bold disabled:opacity-40">›</button>
        </div>
        ...
        {/* Live summary badges from API response */}
        {apiResponse && (
          <div className="flex items-center gap-2 flex-wrap">
            {(summary.daily_count  ?? 0) > 0 && (
              <span className="sd-tab-badge">{summary.daily_count} daily</span>
            )}
            {(summary.period_count ?? 0) > 0 && (
              <span className="sd-tab-badge">{summary.period_count} period</span>
            )}
            {(summary.total_records ?? 0) > 0 && (
              <span className="sd-att-muted text-[12px]">{summary.total_records} total</span>
            )}
          </div>
        )}
      ...
      {/* ── Loading ── */}
      {loading && (
        <p className="sd-att-muted text-[13.5px] text-center py-10">Loading attendance…</p>
      )}

      {/* ── Error ── */}
      {!loading && error && (
        <div className="sd-coming-soon rounded-2xl flex flex-col items-center justify-center gap-3 py-12 text-center">
          <p className="text-[14px] font-semibold" style={{ color:"var(--danger)" }}>
            Failed to load attendance
          </p>
          <p className="sd-att-muted text-[13px]">{typeof error === "string" ? error : JSON.stringify(error)}</p>
          <button onClick={load}
            className="sd-doc-view-btn inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-[13px] font-semibold mt-1">
            <RefreshCw size={13}/> Retry
          </button>
        </div>
      )}

      {/* ── Stats (only when daily data exists) ── */}
      {!loading && !error && dailyRows.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          <div className="sd-att-stat-card rounded-2xl p-4 flex flex-col items-center justify-center gap-1">
            <AttendanceRing pct={stats.pct}/>
            <p className="sd-att-stat-label text-[12px] text-center mt-1">
      ...
      {/* ── No data ── */}
      {!loading && !error && apiResponse && !hasData && (
        <div className="sd-coming-soon rounded-2xl flex flex-col items-center justify-center gap-3 py-16 text-center">
          <CalendarDays size={36} className="sd-att-muted opacity-40"/>
          <p className="sd-field-value text-[14px] font-semibold">No attendance records for this period</p>
          <p className="sd-att-muted text-[13px]">Try selecting a different month or attendance type</p>

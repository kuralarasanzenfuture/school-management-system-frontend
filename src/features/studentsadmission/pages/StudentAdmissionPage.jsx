...
import StudentAdmissionTable from "../components/StudentAdmissionTable.jsx";
import StudentAdmissionModal from "../components/StudentAdmissionModal.jsx";
import DeleteStudentAdmissionModal from "../components/DeleteStudentAdmissionModal.jsx";
import StatusFilterDropdown from "../components/StatusFilterDropdown";
import {
  fetchStudentAdmissions,
  createStudentAdmissionThunk,
...
const StudentAdmissionPage = () => {
  ...
  // StudentAdmissionTable calls onDelete(id) — matching every other table
  // in this app. DeleteStudentAdmissionModal needs the full record
  // (admission_number, class_name, student_id) to show a meaningful
  ...
  const openDeleteModal = (id) => {
    const admission = studentAdmissions.find(
      (a) => String(a.id) === String(id),
    );
    setDeleteTarget(admission ?? null);
  };

  const closeDeleteModal = () => {
    setDeleteTarget(null);
    dispatch(fetchStudentAdmissions()); // reload list after a successful delete
  };

  const deleteTargetStudent = deleteTarget
    ? studentsById[deleteTarget.student_id]
    : null;
  const deleteTargetStudentName = deleteTargetStudent
    ? `${deleteTargetStudent.first_name || ""} ${deleteTargetStudent.last_name || ""}`.trim()
    : "";
  ...
  return (
    <div className="sa-page min-h-screen p-6">
      ...
      <DeleteStudentAdmissionModal
        isOpen={Boolean(deleteTarget)}
        onClose={closeDeleteModal}
        admission={deleteTarget}
        studentName={deleteTargetStudentName}
      />
    </div>
  );
};
